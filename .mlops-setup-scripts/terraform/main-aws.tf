...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = "mlops-stack-aws-seek-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "mlops-stack-aws-seek-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for mlops-stack-aws-seek"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "mlops-stack-aws-seek-terraform-cicd-user"
}
...
resource "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "mlops-stack-aws-seek-cicd-setup-tfstate"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "mlops-stack-aws-seek-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
