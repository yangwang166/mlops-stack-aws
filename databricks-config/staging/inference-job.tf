resource "databricks_job" "batch_inference_job" {
  name = "${local.env_prefix}mlops-stack-aws-seek-batch-inference-job"
  ...
  notebook_task {
    ...
    base_parameters = {
      ...
      input_table_name  = "mlops_stack_yang_seek.inference_data"
      output_table_name = "mlops_stack_yang_seek.result_staging"
    }
  }
